...
String sendMWV(int sendmode) {
	...
	if ((int(actconf.serverMode) == 1 && sendmode == 1)
			|| (int(actconf.debug) >= 3 && sendmode == 1)) {
...
String sendVPW(int sendmode) {
	...
	if (winddirection >= (180 - float(actconf.downWindRange))
			&& winddirection <= (180 + float(actconf.downWindRange))) {
...
String sendMDA(int sendmode) {

	String HexCheckSum;
	String NMEAMDA;
	String SendMDA;

	// Create NMEA string for wind speed $WIMDA,,,x.x,B,x.x,C,,,x.x,,x.x,C,,,,,,,*hh<CR><LF>
	NMEAMDA = "WIMDA,,,";
	NMEAMDA += String(airpressure / 1000.0, 7); //the nmea sentence expect it in Bars
	NMEAMDA += ",B,";
	NMEAMDA += String(airtemperature);
	NMEAMDA += ",C,,,";

	NMEAMDA += String(airhumidity);
	NMEAMDA += ",,";
	NMEAMDA += String(dewpoint);
	NMEAMDA += ",C,,,,,,,,,";
//	NMEAMDA += ",C,";
//	NMEAMDA += String(winddirection);
//	NMEAMDA += ",T,,,,,";
//	NMEAMDA += String(windspeed_mps);
//	NMEAMDA += ",M";

	// NMEAMDA +=  String(altitude);
	//NMEAMDA +=  ",M,";
	// Build CheckSum
	HexCheckSum = String(CheckSum(NMEAMDA), HEX);
	// Build complete NMEA string
	SendMDA = "$" + NMEAMDA;
	SendMDA += "*";
	SendMDA += HexCheckSum;
