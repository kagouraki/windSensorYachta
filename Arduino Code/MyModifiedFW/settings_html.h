...
//    }
    if (vname[i] == "offset") {
      if (toInteger(value[i]) >= 0 && toInteger(value[i]) <= 360){
        actconf.offset = toInteger(value[i]);
      }
      if (toInteger(value[i]) < 0){
        actconf.offset = 0;
      }
      if (toInteger(value[i]) > 360){
        actconf.offset = 360;
      }  
    }
    if (vname[i] == "average") {
 ...
 if(actconf.crypt == 1 && (hash.length() == 0 || hash != cryptPassword(String(actconf.password)))){
  ...
  // content += CSS();
   ...
   transID();
    ...
    content += F("<option value='1'>1s</option>");
    ...
    content += F("<option value='57600'>16 hours</option>");
    content += F("</select>");
    content += F("</td>");
    content += F("<td></td>");
    content += F("</tr>");
