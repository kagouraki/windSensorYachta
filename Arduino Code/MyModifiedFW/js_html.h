...
String JS() {
	...
	content +=
			F(
					"document.getElementById('hunit').innerHTML = myObj.Device.ESP8266.FreeHeapSize.Unit;");
	...
	if (String(actconf.tempSensorType) == "DS18B20") {
		...
		content +=
				F(
						"temp.value = myObj.Device.MeasuringValues.DeviceTemperature.Value;");
		content +=
				F(
						"document.getElementById('tunit').innerHTML = myObj.Device.MeasuringValues.DeviceTemperature.Unit;");
	}

	content += F("winddir = document.getElementById('winddir');");
	content +=
			F(
					"winddir.value = myObj.Device.MeasuringValues.WindDirection.Value;");
	content +=
			F(
					"document.getElementById('dirunit').innerHTML = myObj.Device.MeasuringValues.WindDirection.Unit;");
	content += F("resolution = document.getElementById('resolution');");
	content +=
			F(
					"resolution.value = myObj.Device.MeasuringValues.Resolution.Value;");
	content +=
			F(
					"document.getElementById('resunit').innerHTML = myObj.Device.MeasuringValues.Resolution.Unit;");
	content += F("windspeed = document.getElementById('windspeed');");
	content += F(
			"windspeed.value = myObj.Device.MeasuringValues.WindSpeed.Value;");
	content +=
			F(
					"document.getElementById('speedunit').innerHTML = myObj.Device.MeasuringValues.WindSpeed.Unit;");
	content += F("dwspeed = document.getElementById('dwspeed');");
	content +=
			F(
					"dwspeed.value = myObj.Device.MeasuringValues.DownWindSpeed.Value;");
	content +=
			F(
					"document.getElementById('dwunit').innerHTML = myObj.Device.MeasuringValues.DownWindSpeed.Unit;");
	content += F("sensor1 = document.getElementById('sensor1');");
	content += F("sensor1.value = myObj.Device.MeasuringValues.Sensor1.Value;");
	content +=
			F(
					"document.getElementById('s1unit').innerHTML = myObj.Device.MeasuringValues.Sensor1.Unit;");
	if (String(actconf.windSensorType) == "WiFi 1000") {
		content += F("sensor2 = document.getElementById('sensor2');");
		content += F(
				"sensor2.value = myObj.Device.MeasuringValues.Sensor2.Value;");
		content +=
				F(
						"document.getElementById('s2unit').innerHTML = myObj.Device.MeasuringValues.Sensor2.Unit;");
		content += F("pcounter = document.getElementById('pcounter');");
		content +=
				F(
						"pcounter.value = myObj.Device.MeasuringValues.PulseCounter.Value;");
		content +=
				F(
						"document.getElementById('pcunit').innerHTML = myObj.Device.MeasuringValues.PulseCounter.Unit;");
		content += F("time1 = document.getElementById('time1');");
		content += F("time1.value = myObj.Device.MeasuringValues.Time1.Value;");
		content +=
				F(
						"document.getElementById('t1unit').innerHTML = myObj.Device.MeasuringValues.Time1.Unit;");
		content += F("time2 = document.getElementById('time2');");
		content += F("time2.value = myObj.Device.MeasuringValues.Time2.Value;");
		content +=
				F(
						"document.getElementById('t2unit').innerHTML = myObj.Device.MeasuringValues.Time2.Unit;");
	}
	if (String(actconf.windSensorType) == "Yachta"
			|| String(actconf.windSensorType) == "Jukolein"
			|| String(actconf.windSensorType) == "Ventus") {
		content += F("magnitude = document.getElementById('magnitude');");
		content +=
				F(
						"magnitude.value = myObj.Device.MeasuringValues.MagFluxDensity.Value;");
		content +=
				F(
						"document.getElementById('magnitudeunit').innerHTML = myObj.Device.MeasuringValues.MagFluxDensity.Unit;");

		content += F("magsensor = document.getElementById('magsensor');");
		content +=
				F(
						"magsensor.value = myObj.Device.MeasuringValues.MagnetSensor.Value;");
		content +=
				F(
						"document.getElementById('magunit').innerHTML = myObj.Device.MeasuringValues.MagnetSensor.Unit;");
	}
	...
	if (String(actconf.tempSensorType) == "BME280") {
		...
		content +=
				F(
						"atemp.value = myObj.Device.MeasuringValues.AirTemperature.Value;");
		content +=
				F(
						"document.getElementById('aunit').innerHTML = myObj.Device.MeasuringValues.AirTemperature.Unit;");

		content += F("pres = document.getElementById('pres');");
		content += F(
				"pres.value = myObj.Device.MeasuringValues.AirPressure.Value;");
		content +=
				F(
						"document.getElementById('punit').innerHTML = myObj.Device.MeasuringValues.AirPressure.Unit;");

		content += F("hum = document.getElementById('hum');");
		content += F(
				"hum.value = myObj.Device.MeasuringValues.AirHumidity.Value;");
		content +=
				F(
						"document.getElementById('humunit').innerHTML = myObj.Device.MeasuringValues.AirHumidity.Unit;");
	}
	content += F("rotspeed = document.getElementById('rotspeed');");
	content +=
			F(
					"rotspeed.value = myObj.Device.MeasuringValues.RotationSpeed.Value;");
	content +=
			F(
					"document.getElementById('rotunit').innerHTML = myObj.Device.MeasuringValues.RotationSpeed.Unit;");
	/*
