...
// Enviromental readings      $WIMDA,,,x.x,B,x.x,C,,,x.x,,x.x,C,,,,,,,*hh
//
//*********************************************************************************************
...
AS5600 ams5600;           // Declare magnetic rotation sensor AS5600
...
const int maxTCPclients = 5;
WiFiClient serverClients[maxTCPclients];
...
#include "ServerPages.h"    // Webserver pages request functions
	...
	delay(4000);
	// Start interrupts in falling slope modeserver
...
void loop() {
	...
	if (server.hasClient()) {
		...
		int k;
		for (k = 0; k < maxTCPclients; k++)
			if (!serverClients[k]) { // equivalent to !serverClients[i].connected()
				serverClients[k] = server.available();
				DebugPrintln(3, "New client: index ");
				DebugPrintln(3, k);
				break;
			}
...
//
	...
	if (windspeed_mps > 0 && flag1 == true) {
		...
		DebugPrintln(3, "");
		DebugPrintln(3, "Send package:");

		if ((int(actconf.serverMode) == 0) || (int(actconf.serverMode) == 1)
				|| (int(actconf.serverMode) == 4)) {
			if (int(actconf.windSensor) == 1) {

				for (int j = 0; j < maxTCPclients; j++) {

					serverClients[j].println(sendMWV(1));
					serverClients[j].println(sendVWR(1));
			...
			if (int(actconf.tempSensor) == 1) {
				if (String(actconf.tempSensorType) == "DS18B20") {
					for (int j = 0; j < maxTCPclients; j++) {
						...
						serverClients[j].println(sendWST(1));
				...
				if (String(actconf.tempSensorType) == "BME280") {
					for (int j = 0; j < maxTCPclients; j++) {
						...
						serverClients[j].println(sendMDA(1));
