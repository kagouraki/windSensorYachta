...
typedef struct {
  ...
  int apchannel = 9;                        // Assess Point channel [1...13]
  int maxconnections = 4;                   // Max number of connection for WiFi clients [1...4]
  int mDNS = 0;                             // Using mDNS service [0|1] 0=off, 1=on
  ...
  int downWindSensor = 0;                   // Send data to down wind 0=off 1=on (WIVPW)
  ...
  char tempSensorType[10] = "BME280";          // Type of temperature sensor [Off|DS18B20|BME280]
